...
public class Queen extends Piece {
    ...
    private boolean isQueen(Position position) {
        return isDiagonal(position) || this.position.isColumnEquals(position) || this.position
            .isRowEquals(position);
    }

    private boolean isDiagonal(Position position) {
        return Math.abs(position.getColumnGap(this.position)) == Math
            .abs(position.getRowGap(this.position));
    }
