...
public class TurnInfoDAO {
	...
	public Team findCurrent(String gameId) {
		SelectTemplate query = new SelectTemplate("SELECT current_team FROM turn_info WHERE game_id = ?") {
			@Override
			protected void setParameters(PreparedStatement pstmt) throws SQLException {
				pstmt.setString(1, gameId);
			}
	...
	public void updateNext(String gameId) {
		ApplyTemplate query = new ApplyTemplate("UPDATE turn_info SET current_team = ? WHERE game_id = ?") {
			@Override
			protected void setParameters(PreparedStatement pstmt) throws SQLException {
